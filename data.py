...
def get_data_split(ds, seed):
    ...
    path = f"{current_dir}/dataset_descriptions/{source}{ds[0]}.txt"
    try:
        with open(path) as f:
            ds[-1] = f.read()
    except:
...
def load_kaggle():
    ...
    for name in kaggle_dataset_ids:
        try:
            df_all = pd.read_csv(f"{current_dir}/datasets_kaggle/{name[0]}/{name[1]}.csv")
            ...
            data_dir = os.environ.get("DATA_DIR", "data/")
            path = f"{current_dir}/dataset_descriptions/kaggle_{name[0]}.txt"
            try:
                with open(path) as f:
                    ds[-1] = f.read()
            except:
    ...
    for name in kaggle_competition_ids:
        try:
            df_all = pd.read_csv(f"{current_dir}/datasets_kaggle/{name}/train.csv")
            df_train, df_test = train_test_split(df_all, test_size=0.25, random_state=0)
            ds = [
                "kaggle_" + name,
            ...
            path = f"{current_dir}/dataset_descriptions/kaggle_{name}.txt"
            print(path)
            try:
                with open(path) as f:
                    ds[-1] = f.read()
